...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        self.graph = tf.Graph()  # 计算图，主要用于构建网络，本身不进行任何实际的计算
        with self.graph.as_default():
            tf.set_random_seed(self.random_seed)
            # 特征的index，主要用于通过embedding_lookup查询特征向量
            self.feat_index = tf.placeholder(tf.int32, shape=[None, None],
                                             name="feat_index")  # None * F
            # 对应的特征值，数值型，为原来值，离散特征是对应位置的1
            ...
            #  最后，self.embeddings存放的是特征值和隐向量的乘积
            self.embeddings = tf.multiply(self.embeddings, feat_value)    # element_wise
            ...
            # second order tf.subtract函数返回一个Tensor,与 x 具有相同的类型.
            self.y_second_order = 0.5 * tf.subtract(self.summed_features_emb_square,
                                                    self.squared_sum_features_emb)  # None * K
            self.y_second_order = tf.nn.dropout(self.y_second_order, self.dropout_keep_fm[1])  # None * K

            # ---------- Deep component ----------
            self.y_deep = tf.reshape(self.embeddings, shape=[-1, self.field_size * self.embedding_size])  # 二维向量转化为一维
            self.y_deep = tf.nn.dropout(self.y_deep, self.dropout_keep_deep[0])    # 随机失活部分输入神经元
            ...
            # 代码中输出了模型的 Normalization 的 Gini值来进行模型评价，我们可以对比一下(记住，Gini值越大越好)
            # ----------- optimizer  --------------------
    ...
    def _initialize_weights(self):
        weights = dict()          # 网络层中，不同结构中的权重向量矩阵
        """
        权重构建，权重的设定主要有两部分，第一部分是从输入到embedding中的权重，其实也就是我们的dense vector
        另一部分就是深度神经网络中每一层的权重
        """
        ...
        # deep layers 中个各层的权重矩阵
        num_layer = len(self.deep_layers)
        input_size = self.field_size * self.embedding_size
        glorot = np.sqrt(2.0 / (input_size + self.deep_layers[0]))
        weights["layer_0"] = tf.Variable(
            np.random.normal(loc=0, scale=glorot, size=(input_size, self.deep_layers[0])), dtype=np.float32)
        # scale: 概率分布的标准差，对应于分布的宽度，scale越大越矮胖，scale越小，越瘦小
