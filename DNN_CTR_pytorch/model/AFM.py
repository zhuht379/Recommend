...
# 论文解读：https://zhuanlan.zhihu.com/p/82299967
# 2. https://blog.csdn.net/qq_18293213/article/details/90511534     attention 公式介绍
class AFM(torch.nn.Module):
    ...
    def forward(self, Xi, Xv):
        ...
        # 特征交叉项
        fm_wij_arr = []  # emb 之间的元素点积生成长度为emb_dim
        ...
        #  [field_size * (field_size - 1) / 2,embedding_size]
        attention_tmp = self.attention_linear_1(interaction_layer.view([-1, self.embedding_size]))
        # [field_size * (field_size - 1) / 2,embedding_size] * [embedding_size,1]
        attention_tmp = attention_tmp * self.H
        # [1,field_size * (field_size - 1) / 2]
        # 符号“*”对于array类型是矩阵乘法，对于mat类型是按元素类型相乘，而当一个array类型乘上一个mat类型时，则为矩阵乘法；
