...
# 算法详解：
# 1. https://zhuanlan.zhihu.com/p/92293407
# 2. https://blog.csdn.net/needlyr/article/details/111410104
...
class NFM(torch.nn.Module):
    def __init__(self, field_size, feature_sizes, embedding_size=4, is_shallow_dropout=True, dropout_shallow=[0.5],
                 ...
                 use_fm=True, loss_type='logloss', eval_metric=roc_auc_score,
        ...
        # 特征交叉+池化+hidden layers
        if self.is_deep_dropout:
            self.linear_0_dropout = nn.Dropout(self.dropout_deep[0])
        # 第一层DNN
        self.linear_1 = nn.Linear(self.embedding_size, deep_layers[0])  # 特征交叉+池化向量维度等于隐向量的维度
    ...
    def forward(self, Xi, Xv):
        ...
        # use 2xy = (x+y)^2 - x^2 - y^2 reduce calculation  特征交叉+池化
    ...
    def fit(self, Xi_train, Xv_train, y_train, Xi_valid=None, Xv_valid=None,
        ...
        # Xi_train：每一行表示每个特征域中one-hot非零特征的对应的index，因此每一行的维度为field_size
        """
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(1):
        nfm = NFM(39, result_dict['feature_sizes'], batch_size=128 * 64, is_shallow_dropout=False, verbose=True,
                  use_cuda=True,
                  weight_decay=0.00002, use_fm=True, interation_type=False).cuda()
