...
class SparseFeat(namedtuple('SparseFeat',
                            ['name', 'vocabulary_size', 'embedding_dim', 'use_hash', 'dtype', 'embedding_name',
                             'group_name'])):
    __slots__ = ()    # 当一个类需要创建大量实例时，可以通过__slots__声明实例所需要的属性，
    ...
    def __new__(cls, name, vocabulary_size, embedding_dim=4, use_hash=False, dtype="int32", embedding_name=None,
                group_name=DEFAULT_GROUP_NAME):    # cls表示这个类本身
    ...
    def __hash__(self):
        return self.name.__hash__()     # 取hash值

# VarLenSparseFeat的shape是maxlen，即序列最大长度，用户自己设置。
...
"""
 DenseFeat的维度是dimension，即用户设置的维度，由于Dense特征不需要考虑one-hot，所以直接存放embedding后的维度即可.
"""
...
def build_input_features(feature_columns):
    ...
    features = OrderedDict()   # 初始化一个字典存放输入特征
