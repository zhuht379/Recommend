...
"""
https://www.jianshu.com/p/86ffe7e740cc
https://zhuanlan.zhihu.com/p/136210985

"""
# 捕捉用户兴趣的动态变化
# 与DIN类似，输入为四个部分：用户行为序列，目标广告，上下文特征，用户画像特征，利用Embedding层把这四个部分转化为向量表示。
...
class DIEN(BaseModel):
    ...
    def _split_columns(self):
        self.sparse_feature_columns = list(
            filter(lambda x: isinstance(x, SparseFeat), self.dnn_feature_columns)) if len(            # 稀疏特征
            self.dnn_feature_columns) else []
        self.dense_feature_columns = list(
            filter(lambda x: isinstance(x, DenseFeat), self.dnn_feature_columns)) if len(             # 稠密特征
            self.dnn_feature_columns) else []
        self.varlen_sparse_feature_columns = list(
            filter(lambda x: isinstance(x, VarLenSparseFeat),                                         # 真实序列长度
...
"""
依据用户历史序列捕捉实时兴趣，利用GRU对用户行为的依赖关系进行建模，兴趣导致了用户的连续行为序列，为了捕捉这种依赖关系，提出一种loss，利用用户的下一个行为来
监督学习当前的兴趣状态（隐藏层），这种额外信息可以帮助我们捕捉兴趣的语义信息，让GRU隐藏层更高效的表示兴趣。
这一部分从用户行为序列中提取一系列用户状态，利用GRU模型来捕捉这种时序关系，GRU可以解决RNN的梯度消失问题，并且比LSTM速度快，比较适用于电子商务系统。
模型输入为用户的点击(行为)序列it , 输出为隐藏层的兴趣表示 ht , 
"""
# 兴趣提取层
class InterestExtractor(nn.Module):
    ...
    # 由于当前的兴趣和下次的点击行为相关性较强，即每一步的兴趣状态导致连续的用户行为，所以考虑用t+1步的用户行为来监督学习当前t步的兴趣状态h_t, 建立损失函数
    def _cal_auxiliary_loss(self, states, click_seq, noclick_seq, keys_length):
        ...
        return loss
"""
捕捉和目标项相关的兴趣演化过程，在序列结构中引入注意力机制，在兴趣演化的过程中，相关项的影响被加强，在模型上，简单来说，
就是在经典的Embedding & MLP 模型基础上加入GRU与Attention，并修改了损失函数。

用户兴趣是多元的，在邻接的访问中，用户的目的也不尽相同，并且一个用户的行为可能依赖于很长时间前的行为；因此，我们认为，每个兴趣都有它自己的演化进程，
同时，用户在不同项目上的点击行为受到不同兴趣的影响，我们对目标项相关的兴趣演化轨迹进行建模，设计了基于注意力机制的更新门GRU，利用兴趣门和目标项的
关联，AUGRU增强了在兴趣演化中与目标项相关兴趣的影响，弱化了不相关兴趣的影响，这种注意力机制可以在不同的目标项中有不同的演化进程。
https://zhuanlan.zhihu.com/p/136210985
"""
